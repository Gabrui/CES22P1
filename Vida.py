...
class Vida(Camada):
    ...
    def reduzPV(self, Dano):
        ...
        self.filhos[1].corte.setRetangulo(
            self.filhos[1].corte.getTopoEsquerdo(), novoFundoDireito)
    ...
    def atualiza(self, dt):
        ...
        if isinstance(self.Dono, IA):
            ...
            self.pos.setXY(self.posX_barra_vida_jogador,
                           self.posY_barra_vida_jogador)
        super().atualiza(dt)
...
class Altimetro(Figura):
    ...
    def atualizaAltitude(self, Dano):
        ...
        for i in range(1, len(self.string_imagem)):
            fracao_altitude = (len(self.string_imagem) -
                               i) / len(self.string_imagem)
